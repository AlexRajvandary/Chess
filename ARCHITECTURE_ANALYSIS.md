...
sequenceDiagram
    ...
    Note over UI,GameEngine: ✅ Правильный подход
    ...
    Note right of GameEngine: Вся логика внутри:<br/>- Валидация<br/>- Выполнение<br/>- Обновление состояния<br/>- Проверка шах/мат
    ...
    alt Move successful
        ...
        Note right of UI: Обновляет UI на основе<br/>нового состояния
        deactivate UI
    end
```
...
classDiagram
    ...
    class Game {
        ...
        +GameField GameField ⚠️
        +int CurrentPlayer
        +List~IPiece~ Pieces ⚠️
        +List~Player~ Players ⚠️
        +bool IsGameOver
        ...
        -GetGameField(List~IPiece~) string[,] ⚡
    }
    ...
    class MoveValidator {
        ...
        +CanCastle(...) bool ⚡
        +FilterValidMoves(...) List~Position~ ⚡
    }
...
sequenceDiagram
    ...
    Note over WPF,UpdateGameField: ⚡ Проблема: Каскадные вызовы и пересчеты
    ...
    GetGameField->>GetGameField: Создает string[8,8]
    GetGameField->>GetGameField: Перебирает все фигуры
    GetGameField-->>-Game: string[,]
    deactivate GetGameField
    
    Game->>+Piece: AvailableMoves(gameFieldString)
    activate Piece
    Piece->>Piece: Пересчет возможных ходов
    Piece-->>-Game: List~Position~
    deactivate Piece
    
    Game->>+GameField: UpdateGameField()
    activate GameField
    GameField->>GameField: Очищает все клетки
    GameField->>GameField: Для каждой фигуры:<br/>GetAtackStatus()
    GameField->>GameField: Пересчет атак
    deactivate GameField
    Game-->>-WPF: List~Position~
    deactivate Game
    deactivate WPF
    
    Note over WPF,UpdateGameField: При каждом вызове - полный пересчет!
```
...
classDiagram
    ...
    class ISpecialMoveStrategy {
        ...
        ✅ Специальные ходы (En Passant, рокировка)
    }
    
    class EnPassantStrategy {
        +GetSpecialMoves(IPiece, IGameState) List~Position~
        ✅ Стратегия En Passant
    }
    ...
    class IBoardQuery {
        ...
        ✅ Абстракция доски для стратегий
    }
    ...
    class BoardState {
        ...
        +GetPieceAt(Position) IPieceInfo
        +IsCellFree(Position) bool
        ✅ Может быть заменен на bitboards
    }
...
```mermaid
graph TB
    subgraph WPF["🖥️ WPF Application"]
        direction TB
        MW["📱 MainWindow"]
        MV["🎛️ MainViewModel"]
        GV["🎮 GameViewModel<br/>⚠️ Содержит бизнес-логику"]
        BV["📊 BoardViewModel"]
        
        subgraph WPF_Services["⚙️ WPF Services"]
            CGS["🔄 ChessGameService<br/>⚠️ Лишний слой"]
        end
        
        BSS["📦 BoardStateSnapshot<br/>⚠️ Дублирует GameState"]
    end
    
    subgraph ChessLib["♟️ ChessLib Domain"]
        direction TB
        G["🎯 Game<br/>⚠️ Публичные свойства<br/>⚡ Каскадные вызовы"]
        GS["📋 GameState"]
        GF["🗺️ GameField<br/>⚡ Пересчеты"]
        
        subgraph Pieces["♟️ Pieces"]
            IP["🔷 IPiece"]
        end
    end
...
classDiagram
    ...
    ChessGameService --> Game : 🔴 прямая зависимость
    ChessGameService --> BoardStateSnapshot : создает
    Game --> GameState
    Game --> GameField
    Game --> IPiece
    Game --> MoveValidator
    Game --> MoveExecutor
    BoardStateSnapshot ..> GameState : дублирует данные
...
classDiagram
    ...
    GameViewModel --> BoardViewModel
    BoardViewModel --> CellViewModel
    BoardViewModel --> IGameState : ✅ использует напрямую
    ...
    IMoveStrategy --> IBoardQuery : использует
...
graph TB
    subgraph WPF["🖥️ WPF Application"]
        ...
        MW["📱 MainWindow"]
        MV["🎛️ MainViewModel"]
        GV["🎮 GameViewModel<br/>✅ Только UI логика"]
        BV["📊 BoardViewModel"]
    end
    
    subgraph Application["🔌 Application Layer"]
        direction TB
        IGS["📐 IGameService<br/>✅ Абстракция<br/>&lt;&lt;interface&gt;&gt;"]
        GS_Impl["⚙️ GameService<br/>✅ Реализация"]
    end
    
    subgraph ChessLib["♟️ ChessLib Domain"]
        direction TB
        IGE["🎯 IGameEngine<br/>✅ Абстракция<br/>&lt;&lt;interface&gt;&gt;"]
        GE["⚙️ GameEngine<br/>✅ Инкапсулированная<br/>реализация"]
        IGS_State["📋 IGameState<br/>✅ Абстракция состояния"]
        Cache["💾 StateCache<br/>✅ Кэширование"]
        
        subgraph Board["🗺️ Board Abstraction"]
            IBS["📐 IBoardState"]
            IBR["📐 IBoardRepresentation"]
            ABR["⚙️ ArrayBoardRep<br/>Текущая реализация"]
            BBR["⚙️ BitboardRep<br/>🔮 Будущая оптимизация"]
        end
    end
...
### 📍 Этап 3: Рефакторинг Game → GameEngine с CQRS + Strategy Pattern ✅ ЗАВЕРШЕН
...
1. **Создать интерфейс IMoveStrategy и стратегии**
   - [x] Создать `IMoveStrategy` с методами `GetPossibleMoves()` и `GetPossibleCaptures()`
   - [x] Создать стратегии для каждой фигуры: `PawnMoveStrategy`, `KingMoveStrategy`, `QueenMoveStrategy`, `RookMoveStrategy`, `BishopMoveStrategy`, `KnightMoveStrategy`
   - [x] Перенести логику из `AvailableMoves()`/`AvailableKills()` в стратегии
   - [x] Создать `IMoveStrategyRegistry` для регистрации стратегий

2. **Создать интерфейс IBoardQuery**
   - [x] Создать `IBoardQuery` с методами `GetPieceAt()`, `IsCellFree()`, `IsCellAttacked()`
   - [x] Стратегии используют `IBoardQuery` вместо `string[,]`

3. **Создать класс GameEngine**
   - [x] Реализовать `IGameEngine`
   - [x] Сделать все внутренние поля private
   - [x] Убрать публичные свойства `Pieces`, `GameField`, `Players`

4. **Добавить кэширование состояния**
   - [x] Создать `IMoveCache` и `MoveCache` для кэширования ходов
   - [x] Реализовать кэширование в `MoveStrategyService`
   - [x] Инвалидировать кэш только после хода

5. **Обновить IPiece**
   - [x] Убрать методы `AvailableMoves()`/`AvailableKills()` из `IPiece`
   - [x] Оставить только свойства: `Color`, `Position`, `IsDead`

6. **Интеграция стратегий**
   - [x] Обновить `Game` для использования `MoveStrategyService`
   - [x] Обновить `MoveValidator` для использования стратегий
   - [x] Обновить `GameField` для использования стратегий
   - [x] Обновить `AlgebraicNotation` для использования стратегий
...
- ⚠️ Некоторые тесты требуют исправления (игнорируются по требованию)
...
- `IMoveStrategy`, `IMoveStrategyRegistry`, `MoveStrategyRegistry`
- `PawnMoveStrategy`, `KingMoveStrategy`, `QueenMoveStrategy`, `RookMoveStrategy`, `BishopMoveStrategy`, `KnightMoveStrategy`
- `IBoardQuery`, `BoardQuery`
- `GameEngine` (реализует `IGameEngine`)
- `IMoveCache`, `MoveCache`
- `MoveStrategyService` (оркестрирует использование стратегий и кэширование)
...
| Этап | Название | Приоритет | Время | Риски | Статус |
|------|----------|-----------|-------|-------|--------|
| 1 | Подготовка и инфраструктура | 🔴 Критический | 2-4 ч | Низкие | ✅ Завершен |
| 2 | Создание абстракций в библиотеке | 🔴 Критический | 2-3 ч | Низкие | ✅ Завершен |
| 3 | Рефакторинг Game → GameEngine (CQRS) | 🔴 Критический | 6-8 ч | Средние | ✅ Завершен |
| 4 | Устранение дублирования логики ходов | 🟡 Высокий | 4-6 ч | Средние | ⏳ В ожидании |
| 5 | Методы восстановления игры | 🟡 Высокий | 4-6 ч | Средние | ⏳ В ожидании |
| 6 | Абстракция BoardState | 🟡 Высокий | 3-4 ч | Средние | ⏳ В ожидании |
| 7 | Создание IGameService | 🟡 Высокий | 2-3 ч | Низкие | ⏳ В ожидании |
| 8 | Миграция WPF | 🟡 Высокий | 2-3 ч | Средние | ⏳ В ожидании |
| 9 | Удаление устаревшего кода | 🟢 Средний | 1-2 ч | Низкие | ⏳ В ожидании |
| 10 | Финальная проверка | 🔴 Критический | 2-4 ч | Низкие | ⏳ В ожидании |
